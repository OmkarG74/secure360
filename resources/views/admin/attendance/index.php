...
function renderAttendanceTable() {
    ...
    pagedRecords.forEach(r => {
        ...
        if (r.check_out_at) {
            ...
            if (r.check_in_at) {
                const diffSecs = Math.max(0, (new Date(r.check_out_at).getTime() - new Date(r.check_in_at).getTime()) / 1000);
            ...
            if (parseInt(r.status, 10) === 0) {
                ...
                if (r.check_in_at) {
                    const diffSecs = Math.max(0, (Date.now() - new Date(r.check_in_at).getTime()) / 1000);
                    const hrs = Math.floor(diffSecs / 3600);
                    const mins = Math.floor((diffSecs % 3600) / 60);
